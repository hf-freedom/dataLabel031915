...
def generate_random_id_card():
    area_codes = ["110101", "310101", "440106", "330102", "320102", "510104", "500101", "420106", "370102", "610102"]
...
def init_excel():
    if not os.path.exists(EXCEL_FILE):
        ...
        ws = wb.active
        ws.title = "注册数据"
        headers = ["序号", "用户名", "密码", "邮箱", "姓名", "年龄", "手机号", "注册时间", "注册状态", "登录状态", "购票姓名", "身份证号", "购票状态"]
        ws.append(headers)
        wb.save(EXCEL_FILE)
...
def perform_ticket_purchase(page, user_data, task_id):
    ...
    if ticket_btn:
        ...
        name_input, _ = find_input(page, name_selectors, "购票姓名")
...
def single_task(task_id, user_data):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print(f"[任务{task_id}] 购票状态: {ticket_result['ticket_status']}")
            print(f"[任务{task_id}] ==============================\n")
